...
/*   Updated: 2023/02/05 21:15:12 by hyeyukim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_philo_state(t_philo *philo, int state)
{
	...
	printf("%zu %d %s\n", \
	get_time_interval(&philo->shared->start_time, &now), philo->id, msg[state]);
	pthread_mutex_unlock(&philo->shared->print_lock);
}

size_t	get_time_interval(struct timeval *start, struct timeval *now)
{
