...
/*   Updated: 2023/02/05 18:14:54 by hyeyukim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			struct timeval *start, struct timeval *end, size_t time)
{
	...
	while (get_time_interval(start, end) + 1 < time)
	{
		usleep(100);
		gettimeofday(end, NULL);
	}
	while (get_time_interval(start, end) < time)
	{
		gettimeofday(end, NULL);
	}
}
...
void	*philo_routine(void *arg)
{
	...
	if (is_single_philo(philo) == TRUE)
		return (NULL);
	if ((philo->id & 1) == 1)
		usleep(philo->shared->time_to_eat * 500);
	while (1)
	{
